...
def make_model():
    ...
    feature_layers = [
        Conv2D(6, kernel_size=(3, 3), padding="valid", activation="relu"),
...
def do_initial_learning(dataDir,outputfile):
    ...
    model = make_model()
    model.summary()
    plot_model(model, to_file='Lenet_model_structure.png', show_shapes=True)
    ...
    for i in range(len(inputDirs)):
        ...
        train_samples = make_sample_list("steering", train_data,expand_data=True, expand_correction=0.5)
...
def main(argv):
    inputfile = 'model_lenet_0'
    outputfile = 'model_lenet_'
    dataDir = './Data'
    initial_learning = True
    transfer_learning = False
    try:
        opts, args = getopt.getopt(argv,"htd:i:o:",["ddir=","ifile=","ofile="])
    except getopt.GetoptError:
        print ('behaviourcloning_lenet.py -h -t [0/1] -d <data_dir> -i <history> -o <outputfile>')
        print ('-h: Write this help message.')
        print ('-t: Transfer learning switch. 0: do not perform, 1: Only transffer learning.')
        ...
        ' Extension h5 is added. Default is model_lenet_.')
        print ('outputfile: Output model file basic name.' +
        ' For each of data directories, a file is saved with this name<counter>.h5.  Default is model_lenet_.')
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print ('behaviourcloning_lenet.py -h -t [0/1] -d <data_dir> -i <history> -o <outputfile>')
